...
import 'package:tasca4/data/comarca.dart';
...
class _InfoComarcaState extends State<Infocomarca> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
        future: peticionsHttp.infoComarca(widget.comarca),
        builder: (BuildContext context, AsyncSnapshot<Comarca> snapshot) {
          if (snapshot.hasData) {
            var comarca = snapshot.data;
            var clima = peticionsHttp.obteClima(
                longitud: comarca!.longitud!,
                latitud: comarca!
                    .latitud!); // Hay que importar la clase comarca que viene del future
